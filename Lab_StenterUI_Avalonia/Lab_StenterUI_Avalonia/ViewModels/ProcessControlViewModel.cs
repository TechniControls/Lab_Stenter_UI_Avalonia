...
{
    public string Title => "Process Control Page";
        ...
        ConnectionStore connectionStore)
    {
        ...
        // Initialize Commands
        SetTemperatureCommand = new RelayCommand(_ => _connectionService.WriteCustomTemperature(CustomTemperature),
            _ => _enableCustomRecipes);
        SetFanSpeedCommand = new RelayCommand(_ => _connectionService.WriteCustomFanSpeed(CustomFanSpeed),
            _ => _enableCustomRecipes);
        SetExtractorSpeedCommand =
            new RelayCommand(_ => _connectionService.WriteCustomExtractorSpeed(CustomExtractorSpeed),
                _ => _enableCustomRecipes);
        SetTimeCommand = new RelayCommand(_ => _connectionService.WriteCustomDurationTime(CustomDurationTime),
            _ => _enableCustomRecipes);

        StartCommand =
            new RelayCommand(
                _ => _connectionService.ReadTemperature(CustomTemperature, _recipesModel.BlondaTemperature),
                _ => IsConnected);
